 ...
 function CoolPush(text, desp) {
   return  new Promise(resolve => {
     if (QQ_SKEY) {
       ...
       // 已知敏感词
       text = text.replace(/京豆/g, "豆豆").replace(/京东/g, '东东').replace(/京喜/g, '京X');
       desp = desp.replace(/jd/g, "xx").replace(/京东/g, '东东').replace(/京豆/g, "豆豆");
       desp = desp.replace(/🐶/g, "");
       desp = desp.replace(/红包/g, "H包");
       desp = desp.replace(/免费使用/g, "");
       desp = desp.replace(/京喜/g, '京X');
